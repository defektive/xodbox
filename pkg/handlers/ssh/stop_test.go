...
// TestDoubleStopIsIdempotent is a regression test for the bug where Stop did
// not nil out h.server, so a second Stop would call Shutdown on an already
// shut-down server. Stop must now nil h.server and both calls return nil.
