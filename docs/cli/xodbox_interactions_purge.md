...
Matching interactions and their captured files are removed permanently; there is no undo. Use --dry-run first to see what would go. The database file is not shrunk on disk -- run the purge worker, which vacuums after deleting, or VACUUM manually.
